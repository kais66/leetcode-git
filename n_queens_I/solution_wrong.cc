...
public:
    ...
    vector<vector<string> > solveNQueens(int n) {
        ...
        solveRec(ret, board, col);
        return ret;
    }

    void solveRec(vector<vector<string> > &ret, vector<int> &board, set<int> &col) {
        int n = board.size();
        // error: this condition is not sufficient: for some n, there's no valid solution.
        ...
        // this generates a compile error - must convert it to value, instead of reference
        for (int &i : col) { // for each column. Now the considered point is (row, i)
            bool valid = true;
            // check diagonal validity
            for (int j=0; j<n; ++j) {
                if (abs(n-j) == abs(i-board[j])) {
                    valid = false;
                    break;
                }
            }
